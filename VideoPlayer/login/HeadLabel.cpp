...
void HeadLabel::slot_setUserHeaderPix(QNetworkReply *reply)
{
    if (reply->error() == QNetworkReply::NoError)
    {
        ...
        pixmap.scaled(QSize(220,220),Qt::KeepAspectRatio,Qt::SmoothTransformation);//必须进行缩放处理
...
//        qDebug() <<QString::fromLocal8Bit("通用头像列表：网络请求图片设置成功！");
    ...
    else//请求失败，加载默认图片
    {
        ...
        this->setPixmap(pixmap);
        m_pix = QPixmap(pixmap).scaled(QSize(220,220),Qt::KeepAspectRatio,Qt::SmoothTransformation);//必须进行缩放处理
        this->setScaledContents(true);
//        emit sig_item_delete();//请求失败，则将创建的item删除
    }
}
