...
//                            "primary key(序号,歌曲名称,文件路径))"));//不能使用中文名
        ...
        query.exec(QString("CREATE TABLE IF NOT EXISTS 'BrowserRecords' (S_id INTEGER PRIMARY KEY AUTOINCREMENT, urlnick varchar(50), url varchar(50));"));
...
{
    QSqlQuery query;
    bool isOK = query.exec(QString("select urlnick, url from BrowserRecords;"));
    if(isOK)
    {
        while (query.next())
        {
            ...
            qDebug() << "urlnick=="<<urlnick<< "url=="<<url;
        }
    }
    else
    {
        qDebug()<< QString::fromLocal8Bit("查找所有记录错误：") << query.lastError();
        return false;
    }
}
...
{
    QSqlQuery query;
    bool isOK = query.exec(QString("insert into BrowserRecords values(%1,'%2','%3')").arg("NULL").arg(urlnick).arg(url));
    if(isOK)
    {
...
{
    QSqlQuery query;
    bool isOK = query.exec(QString("delete from BrowserRecords where url= '%1';").arg(url));
    if(isOK)
    {
...
{
    QSqlQuery query;
    bool isOK = query.exec(QString("update BrowserRecords set urlnick='%1' where url = '%2';").arg(urlnick).arg(url));
    if(isOK)
    {
