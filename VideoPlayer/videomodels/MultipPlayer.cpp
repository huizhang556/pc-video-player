    ...
    m_isHide(false)//侧边栏默认没有隐藏
{
    ...
    this->setWindowTitle(QString(u8"播放器"));
...
//                           QString::fromLocal8Bit("图像文件(*.jpg *.png *bmp *gif *jpeg *psd *svg *ico)")};
    ...
    //重写了QSlider,为了获取任意帧
    ui->horizontalSlider->setFixedHeight(26);
...
//    m_audioRecder->setOutputLocation(QUrl::fromLocalFile("test.amr"));
    ...
    m_videoBlank = new VideoBlank(ui->stackedWidget);
    ...
    m_videoTitleBar = new VideoTitleBar(this);
    ...
    m_listWisget1->setMinimumWidth(LISTWIDTH_R);
    ...
    m_listWisget2->setMinimumWidth(LISTWIDTH_R);
    ...
    m_listWisget3->setMinimumWidth(LISTWIDTH_R);
    ...
    m_listWisget4->setMinimumWidth(LISTWIDTH_R);
    ...
    m_vHlayout->setSpacing(2);//搜索框距离列表2px
...
//    m_verticalLayout_main->setStretch(1,1);
    ...
    m_mainTimer->setInterval(3000);//每3秒执行一次
...
//    });
    ...
    //进度条被定为特殊位置
    connect(ui->horizontalSlider,&CusHSlider::sig_valueToPosition,[=](int pos){
    ...
    connect(m_player,&QMediaPlayer::currentMediaChanged,[=](const QMediaContent &media){
        qDebug() << QString(u8"<--------------------------------当前媒体改变（start）-------------------------------------->") << endl;
        qDebug() << QString(u8"当前媒体id:") << m_curMediaId << endl;
        qDebug() << QString(u8"当前媒体名称:") << m_curMediaName << endl;
        qDebug() << QString(u8"当前媒体url:") << m_curMediaUrl << endl;
        qDebug() << QString(u8"当前媒体头像:") << m_curMediaHeader << endl;
        qDebug() << QString(u8"正式列表当前媒体url:") <<  media.canonicalResource().url() << endl;
        qDebug() << QString(u8"<--------------------------------当前媒体改变（end）--------------------------------------->") << endl;
        slot_setCurrentMediaHeader(m_curMediaId);//加载当前媒体所属用头像(使用处2)
...
//        qDebug() << QString(u8"<--------------------------------临时列表当前媒体改变（start）-------------------------------------->") << endl;
//        qDebug() << QString(u8"当前媒体id:") << m_curMediaId << endl;
//        qDebug() << QString(u8"当前媒体名称:") << m_curMediaName << endl;
//        qDebug() << QString(u8"当前媒体url:") << m_curMediaUrl << endl;
//        qDebug() << QString(u8"当前媒体头像:") << m_curMediaHeader << endl;
//        qDebug() << QString(u8"正式列表当前媒体url:") <<  media.canonicalResource().url() << endl;
//        qDebug() << QString(u8"<--------------------------------临时列表当前媒体改变（end）--------------------------------------->") << endl;
//        slot_setCurrentMediaHeader(m_curMediaId);//加载当前媒体所属用头像(使用处3)
//        setAnyFrameMediaUrl(media.canonicalResource().url());//辨别是否为video,是，设置player2的媒体路径，并开启显示图片
//    });
    ...
    connect(m_player2,&QMediaPlayer::mediaStatusChanged,[=](QMediaPlayer::MediaStatus status){
        if(m_player2->media().isNull()) return;

        if(status == QMediaPlayer::UnknownMediaStatus)//未知媒体状态
        {
            qDebug() << QString::fromLocal8Bit("QMediaPlayer::UnknownMediaStatus");
            ui->horizontalSlider->setLoadingStatus(QString(u8"UnknownMediaStatus"));
        }
        else if(status == QMediaPlayer::NoMedia)//无媒体状态
        {
            qDebug() << QString::fromLocal8Bit("QMediaPlayer::NoMedia");
            ui->horizontalSlider->setLoadingStatus(QString(u8"NoMedia"));
        }
        else if(status == QMediaPlayer::LoadingMedia)//加载媒体中
        {
            qDebug() << QString::fromLocal8Bit("QMediaPlayer::LoadingMedia");
            ui->horizontalSlider->setLoadingStatus(QString(u8"LoadingMedia"));
        }
        else if(status == QMediaPlayer::LoadedMedia)//媒体加载完毕
        {
            qDebug() << QString::fromLocal8Bit("QMediaPlayer::LoadedMedia");
            ui->horizontalSlider->setLoadingStatus(QString(u8"LoadedMedia"));
        }
        else if(status == QMediaPlayer::StalledMedia)//媒体停顿
        {
            qDebug() << QString::fromLocal8Bit("QMediaPlayer::StalledMedia");
            ui->horizontalSlider->setLoadingStatus(QString(u8"StalledMedia"));
        }
        else if(status == QMediaPlayer::BufferingMedia)//媒体正在缓冲
        {
            qDebug() << QString::fromLocal8Bit("QMediaPlayer::BufferingMedia");
            ui->horizontalSlider->setLoadingStatus(QString(u8"BufferingMedia"));
        }
        else if(status == QMediaPlayer::BufferedMedia)//媒体缓冲完毕
        {
            qDebug() << QString::fromLocal8Bit("QMediaPlayer::BufferedMedia");
            ui->horizontalSlider->setLoadingStatus(QString(u8"BufferedMedia"));
        }
        else if(status == QMediaPlayer::EndOfMedia)//媒体结束
        {
            qDebug() << QString::fromLocal8Bit("QMediaPlayer::EndOfMedia");
            ui->horizontalSlider->setLoadingStatus(QString(u8"EndOfMedia"));
            m_player2->pause();
        }
        else if(status == QMediaPlayer::InvalidMedia)//媒体无效
        {
            qDebug() << QString::fromLocal8Bit("QMediaPlayer::InvalidMedia");
            ui->horizontalSlider->setLoadingStatus(QString(u8"InvalidMedia"));
        }
        else
        {
            qDebug() << QString::fromLocal8Bit("other unknow problem!");
            ui->horizontalSlider->setLoadingStatus(QString(u8"unknow problem!"));
        }
    });
    ...
    connect(m_videoSurface,&VideoSurface::frameAvailable,[=](QVideoFrame& frame){
        ...
        ui->horizontalSlider->updateValueToImage(recvImage);
        frame.unmap();
    });
...
//            m_videoTitleBar->clearTitleText();
    ...
    //标题栏--固定
    connect(m_videoTitleBar,&VideoTitleBar::sig_titlefix,[=](bool checked){
        qDebug() << QString(u8"标题栏固定标志：") << checked;
        m_visible_title = checked;
    });
            ...
            QTimer::singleShot(500,[=]()
            {
                ...
                if(!filePath.isEmpty())
                {
                    // 获取所有屏幕
                       ...
                       {
                           screenshot = screen->grabWindow(this->window()->winId(),2,
                                                           m_videoTitleBar->height()+2,
                                                           videoWidget->width(),
                                                           videoWidget->height()-ui->stackedWidget_player->height()-m_videoTitleBar->height());
                       }
                       else
                       {
                           screenshot = screen->grabWindow(this->window()->winId(),2,
                                                           m_videoTitleBar->height()+2,
                                                           videoWidget->width()-m_widget1->width(),
                                                           videoWidget->height()-ui->stackedWidget_player->height()-m_videoTitleBar->height());
                       }
                       // 保存截图到文件
...
//    connect(m_videoTitleBar,&VideoTitleBar::sig_winVClose,[=](){m_adjustBright->close();});
    ...
    connect(playlist,&QMediaPlaylist::currentIndexChanged,[=](int index)
    {
        ...
        else//暂时用不到，推荐列表的视频由playlist_t管理
        {
...
//    connect(m_adjustBright,SIGNAL(valueChange_playBackMode(int)),this,SLOT(adjust_playBackMode(int)));
    ...
    //打开文件
    connect(m_actOpenDir,&QAction::triggered,[=](){m_searchBtn->click();});
...
/*判断文件类型2----根据文件名(网络的或者本地的)*/
bool MultipPlayer::fileType(QUrl furl)
{
    m_curMediaUrl = furl.toString();
    //文件的判断以后还可以添加，目前只判断是以http开头的网络文件，还是本地文件、或者其他什么的
    if(furl.toString().startsWith("http",Qt::CaseInsensitive))//不区分大小写，以http开，则认为是网络文件
    {
        m_curMediaName = Base64ToQStr(furl.fileName());//以最后一个/分割，格式为xxx.flv
    }
    else if(furl.isLocalFile())//判断为本地文件
    {
...
//            qDebug()<<"key = "<<it.key()<<"value = "<<it.value();
            if(it.value() == value)
            {
                qDebug() << QString(u8"找到的key:") << it.key();
                return   it.key();
            }
        }
        return -1;
...
bool MultipPlayer::getCurrentFileType(const QString &fpath_name)
{
    QString realName = fpath_name;
    if(fpath_name.startsWith("http",Qt::CaseInsensitive))//不区分大小写，以http开，则认为是网络文件
    {
        realName = realName.remove(QUrl(fpath_name).fileName()) + Base64ToQStr(QUrl(fpath_name).fileName());
        qDebug() << QString(u8"判断为网络文件--文件名为：%1").arg(realName);
    }
    else if(QUrl(fpath_name).isLocalFile())//判断为本地文件
    {
        realName = fpath_name;//本地直接获取文件名
        qDebug() << QString(u8"判断为本地文件--文件名为：%1").arg(realName);
    }
    else
    {
        qDebug() << QString(u8"判断为既不是本地文件，又不是网络文件！");
    }
    QFileInfo fileInfo(realName);
    return getFileType2(fileInfo.suffix());

}

bool MultipPlayer::getFileType2(const QString &fileName)
{
    //判断一下文件类型，加载不同图标
...
bool MultipPlayer::getFileType(const QString &fileSuffix)
{
    //判断一下文件类型，加载不同图标
    if(fileSuffix == "mp4")//返回的是mp4,而非 .mp4
    {
...
//        ui->pushButton_5->setFocusPolicy(Qt::NoFocus);//点击按钮后去掉虚线框
        ...
        if(!m_fileNames.isEmpty())
        {
            ...
            //这里必须加一个定时器，以解决界面缓冲，使得界面来得及反应（主要是标题栏反应不过来）
...
{
    if(slot_getCurrentPlayList()->mediaCount() == 0)
    {
        qDebug() << slot_getCurrentPlayList() << QString(u8"当前列表数量：%1").arg(slot_getCurrentPlayList()->mediaCount());
        return;
    }
...
void MultipPlayer::enterEvent(QEvent *event)
{
    Q_UNUSED(event)
    ...
    if(!isFullScreen())
    {
        ...
        m_videoTitleBar->show();
        updateControlAreaGeomotry();
        ui->stackedWidget_player->show();
    }
...
void MultipPlayer::leaveEvent(QEvent *event)
{
    Q_UNUSED(event)
    this->setCursor(Qt::ArrowCursor);
    if(!isFullScreen())
    ...
    {
        {
            ...
            else if(event->type() == QEvent::MouseButtonDblClick)
            {
                ...
                if(m_player->state() == QMediaPlayer::PlayingState)//左键双击
                {
...
/*音量调节界面显隐*/
void MultipPlayer::volumeAdjustShowUi(QObject *watched, QMouseEvent *event)
{
        /*音量调节显示*/
...
void MultipPlayer::stackWidgetSliderButtonEventFilter(QObject *watched, QMouseEvent *event)
{
    //非全屏的情况
...
/*鼠标移入*/
void MultipPlayer::stackWidget_player_enter(QObject *watched, QMouseEvent *event)
{
//    qDebug() << QString(u8"鼠标进入主播放界面！");
    if(watched == ui->stackedWidget && !isFullScreen() && event->type() == QEvent::Enter)
    {
        this->setCursor(Qt::ArrowCursor);
    }
    //当this被其他界面填充以后，MouseMove只会在四周六出来的边缘检测到
    if(watched == this && !isFullScreen() && event->type() == QEvent::MouseMove)
    {
        updateTitleAreaGeomotry();
        m_videoTitleBar->show();
        updateControlAreaGeomotry();
...
/*鼠标移出*/
void MultipPlayer::stackWidget_player_leave(QObject *watched, QMouseEvent *event)
{
//    qDebug() << QString(u8"鼠标离开主播放界面！");
    if(watched == this && !isFullScreen() && event->type() == QEvent::Leave)
    {
...
QString MultipPlayer::Base64UrlToString(QUrl base64Url)
{
    QFileInfo info(base64Url.toString());
    QString fileName_b64 = info.fileName();
    return Base64ToQStr(fileName_b64);
}
...
//            });
    ...
    {
        qDebug() <<QString(u8"媒体不为视频资源！");
        ui->horizontalSlider->setOpenTipImage(false);
    }
}
...
{
    if(!FloatPlayCtl::getInstance()->isHidden() || isFullScreen()) return;
...
//    }
    ...
    fileType(QUrl(curUrl));//判断视频还是歌曲，判断显示名称以及切换对应的界面
    setCollectBtnShowStatus();//处理所有的item改变时的操作
    slot_updateRateTypeUiLayout();//速率恢复正常
    slot_setCurrentMediaHeader(m_curMediaId);//加载当前媒体所属用头像(使用处1)
}
...
bool MultipPlayer::eventFilter(QObject *watched, QEvent *event)
{
    ...
    volumeAdjustShowUi(watched,mousevent);//音量调节
